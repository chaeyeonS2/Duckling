...
const AvatarDeco = () => {
  ...
  useEffect(() => {
    const getPost = async () => {
      try {
        const response = await axios.get(`https://us-central1-netural-app.cloudfunctions.net/api/assets/${typeDecoState}/${type}`);
  ...
  // };
    ...
    return (
        <div className="layoutDeco">
            <div><HeaderDeco/></div>
            <div className="avatarDeco" style={{width:'100vw', height:'100vh', backgroundImage:"url(/img/home/background.png)"}} >
            <Suspense fallback={null}>
            <Canvas style={{ background: 'transparent' }} ref={canvasRef} 
